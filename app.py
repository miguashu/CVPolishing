...
# ---------- 公网访问防护：登录防爆破 + 全局 API 限流 ----------
# 临时公网（Cloudflare Tunnel）场景，进程内限流；Flask 单进程部署足够，
# 多 worker 部署需换成 Redis 等共享存储。阈值可用环境变量覆盖。
_TRUSTED_PROXIES = ("127.0.0.1", "::1", "localhost")

_LOGIN_ATTEMPTS = {}    # ip -> 失败时间戳列表
_BAN_UNTIL = {}         # ip -> 解封时间戳
LOGIN_LIMIT = int(os.environ.get("LOGIN_LIMIT", "5"))       # 窗口内允许失败次数
LOGIN_WINDOW = int(os.environ.get("LOGIN_WINDOW", "300"))   # 统计窗口（秒，5 分钟）
LOGIN_BAN = int(os.environ.get("LOGIN_BAN", "900"))         # 封禁时长（秒，15 分钟）

_API_HITS = {}          # ip -> 请求时间戳列表
API_LIMIT = int(os.environ.get("API_LIMIT", "60"))     # 窗口内允许请求次数
API_WINDOW = int(os.environ.get("API_WINDOW", "60"))   # 统计窗口（秒，1 分钟）
...
def _client_ip():
    ...
    Cloudflare Tunnel 转发后 request.remote_addr 是 127.0.0.1（隧道本地回环），
    真实外网 IP 位于 Cf-Connecting-Ip / X-Forwarded-For 头。仅当直连来源是
    受信代理（回环地址）时才信任这些头，避免直连时伪造 XFF 绕过限流。
    """
    remote = request.remote_addr
    if remote and remote not in _TRUSTED_PROXIES:
        return remote
    # remote_addr 缺失或为受信代理（如 Cloudflare Tunnel 本地回环）时信任转发头
    for header in ("Cf-Connecting-Ip", "X-Forwarded-For"):
        value = request.headers.get(header, "")
        if value:
            first = value.split(",")[0].strip()
            if first and first not in _TRUSTED_PROXIES:
                return first
    return remote or "unknown"


def _prune_timestamps(timestamps, window):
...
def check_login_rate_limit(ip):
    """登录/注册防爆破：窗口内失败次数达上限即封禁。返回 (code, msg)，code 为 None 表示放行。"""
    now = time.time()
    with _RATE_LOCK:
        ban_until = _BAN_UNTIL.get(ip, 0)
        if ban_until > now:
            return 429, "尝试过于频繁，请 %d 分钟后重试" % (int(ban_until - now) // 60 + 1)
        hits = _LOGIN_ATTEMPTS.setdefault(ip, [])
        _prune_timestamps(hits, LOGIN_WINDOW)
        if len(hits) >= LOGIN_LIMIT:
            _BAN_UNTIL[ip] = now + LOGIN_BAN
            _LOGIN_ATTEMPTS[ip] = []
            return 429, "尝试过于频繁，请 %d 分钟后重试" % (LOGIN_BAN // 60)
    return None, None
...
def check_api_rate_limit(ip):
    """全局 API 滑动窗口限流；登录/注册接口在 before_request 中单独处理。"""
    now = time.time()
    with _RATE_LOCK:
        hits = _API_HITS.setdefault(ip, [])
        _prune_timestamps(hits, API_WINDOW)
...
def _auth_and_ctx():
    ...
    # 公网防护：登录/注册接口防爆破；其余 /api/* 全局滑动窗口限流
    if path in ("/api/login", "/api/register"):
        code, msg = check_login_rate_limit(ip)
        if code:
            return jsonify({"error": msg}), code
    elif path.startswith("/api/"):
        code, msg = check_api_rate_limit(ip)
        if code:
            return jsonify({"error": msg}), code

    if path in ("/", "/interview", "/jd-resume", "/prompts", "/flow"):
        if not username:
...
@app.route("/jd-resume")
def jd_resume_page():
    return send_file(os.path.join(BASE_DIR, "jd_resume.html"))
...
def api_login():
    ...
    clear_login_attempts(_client_ip())
...
def api_resume_post():
    """保存简历：支持整体覆盖（默认）或追加生成的工作经历（append_work）。

    append_work 为 [{company, role, period, bullets}] 时，服务端读取当前简历后
    将内容追加到「工作经历」区域，避免前端并发覆盖写。
    """
    body = _json_body()
    original = body.get("original", "") or ""
    optimized = body.get("optimized", "") or ""
    jd = body.get("jd", "") or ""
    append_work = body.get("append_work")
    append_position = ""
    if append_work:
        optimized, append_position = _append_work_entries(optimized or original, append_work)
    db.save_resume(session["username"], original, optimized, jd)
    resp = {"ok": True}
    if append_work:
...
def _append_work_entries(resume_text, entries):
    """把生成的工作经历条目追加到简历「工作经历」区域。

    返回 (new_text, position)：position 为 "work"（定位到章节后插入）或 "end"（文末追加）。
    章节标题支持中文/英文、Markdown 标题或方括号包裹（忽略大小写）。
    """
    blocks = []
    for e in entries:
        if not isinstance(e, dict):
            continue
        company = str(e.get("company") or "xxx").strip() or "xxx"
    ...
    addition = "\n\n" + "\n\n".join(blocks)
    m = re.search(
        r"(?mi)^[ \t]*(#+)?[ \t]*[【\[]?[ \t]*(工作经历|工作经验|实习经历|项目经历|work experience|professional experience|project experience)[ \t]*[】\]]?[ \t]*$",
        resume_text,
    )
    if m:
        idx = m.end()
        return resume_text[:idx] + addition + resume_text[idx:], "work"
    return ((resume_text.rstrip() + addition) if (resume_text or "").strip() else addition.strip()), "end"
...
def admin_page():
    # 后台系统仅管理员（config.ADMIN_USERS：admin / liwenhao）可访问；
    # 非管理员不踢登录，仅展示无权限提示页
    if not is_admin():
        return '''
        <!doctype html><html lang="zh"><head><meta charset="utf-8">
...
@app.route("/api/jd/resume", methods=["POST"])
def api_jd_resume():
    """SSE 流式：根据 JD 反推生成匹配的工作经历条目，逐条推送。"""
    if not config.LLM_API_KEY:
        return jsonify({"error": "未配置 LLM_API_KEY，无法生成"}), 400
    data = _json_body()
    jd = (data.get("jd") or "").strip()
    resume = (data.get("resume") or "").strip()
    ...
    username = session.get("username", "")
    if not db.is_vip_active(username):
        used = db.get_jd_resume_count(username)
        if used >= config.JD_RESUME_FREE_LIMIT:
            return jsonify({
                "error": f"免费生成次数已用完（{config.JD_RESUME_FREE_LIMIT} 次）。开通 VIP 可无限次生成。",
                "need_vip": True,
                "used": used,
                "limit": config.JD_RESUME_FREE_LIMIT,
            }), 403

    _record_question("jd_resume", f"JD 生成工作经历（{count} 条）", jd[:100])

    def flow():
        ok = False
        for ev in generate_experiences_stream(jd, resume, count=count, target_role=target_role):
            if ev.get("type") == "result":
                ok = True
            yield ev
        if ok and not db.is_vip_active(username):
            db.inc_jd_resume_count(username)
...
def api_interview_generate():
    ...
    _record_question("generate", f"{'补充' if append else '生成'}{count}道面试题", jd[:100])
    return _sse(generate_interview_stream(resume, jd, count=count, enable_search=enable_search,
                                         auto_save=auto_save, append=append, existing=existing))
...
def api_interview_ask():
    ...
    改造为 generator 内完成记忆检索，首条进度事件立即返回，消除用户空白等待。
    """
    if not config.LLM_API_KEY:
    ...
    def ask_flow():
        yield {"type": "progress", "message": "正在检索相关记忆…"}
        ...
        for ev in answer_question_stream(question, original_answer, user_question,
                                          memory_context=memory_context, auto_memory=auto_memory):
            yield ev

    return _sse(ask_flow())
