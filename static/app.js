...
async function runAutoOptimize(taskId) {
  if (running) return;
  // 防御：taskId 只接受非空字符串。点击事件误传 Event 对象时忽略，避免污染请求体。
  if (typeof taskId !== "string" || !taskId.trim()) taskId = null;
  // 会员专属功能门禁
  if (!(window.CVP_USER && window.CVP_USER.vip)) {
...
// 注意：不能直接传 runAutoOptimize，否则点击的 Event 对象会被当作 taskId 参数传入，
// 导致 task_id 变成 {'isTrusted': True} 序列化进请求体，后端 StateRecorder 解析出错。
