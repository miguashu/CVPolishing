...
def init_schema():
    ...
    try:
        with conn.cursor() as c:
            ...
            """)
            # 非会员 JD 生成工作经历次数统计：达上限后提醒开通 VIP（会员不限）
            c.execute("""
                CREATE TABLE IF NOT EXISTS jd_resume_counts (
                  username VARCHAR(64) NOT NULL,
                  cnt INT NOT NULL DEFAULT 0,
                  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                  PRIMARY KEY (username)
...
def get_jd_resume_count(username):
    """返回普通用户已使用的 JD 生成工作经历次数。"""
    conn = connect()
    try:
        with conn.cursor() as c:
            c.execute("SELECT cnt FROM jd_resume_counts WHERE username=%s", (username,))
            row = c.fetchone()
            return int(row["cnt"]) if row else 0
    finally:
        conn.close()
...
def inc_jd_resume_count(username):
    """普通用户成功生成一次工作经历后自增计数（不存在则插入）。"""
    conn = connect()
    try:
        with conn.cursor() as c:
            c.execute(
                "INSERT INTO jd_resume_counts (username, cnt) VALUES (%s, 1) "
                "ON DUPLICATE KEY UPDATE cnt = cnt + 1",
                (username,),
            )
            conn.commit()
