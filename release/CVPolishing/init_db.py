...
def migrate_memory():
    """把现有 data/memory.json 的 118 条长期记忆迁移给 admin（首次运行）。"""
    ...
    user_ctx.set_user_id("admin")
    if get_all():
        print("[init] admin 名下已有长期记忆，跳过迁移")
        return
    ...
    for k, v in data.items():
        ...
        if put(k, answer, extra):
            n += 1
    print(f"[init] 已迁移 {n} 条长期记忆给 admin（向量将在服务启动时按需重建）")
