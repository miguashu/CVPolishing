...
class StateRecorder:
    ...
    def __init__(self, task_id=None):
        ...
        # 防御：task_id 只接受非空字符串。前端点击事件曾把浏览器 Event 对象误当 task_id
        # 传入（task_id={'isTrusted':True}），会导致拼出非法文件路径。非字符串一律视为新建。
        if task_id is None or not isinstance(task_id, str) or not task_id.strip():
